 ...
 * @param  {Meeting}    callback.meeting    The updated meeting object
 */
var startMeeting = module.exports.startMeeting = function(ctx, meetingId, callback) {
    var allVisibilities = _.values(AuthzConstants.visibility);
    ...
    _getMeeting(meetingId, function(err, meeting) {
        ...
        AuthzPermissions.canManage(ctx, meeting, function(err) {
            ...
            MeetingsAPI.emit(MeetingsConstants.events.START_MEETING, ctx, meeting, function(errs) {
                if (errs) {
                    return callback(_.first(errs));
                }
 ...
 * @param  {Meeting}    callback.meeting    The updated meeting object
 */
var endMeeting = module.exports.endMeeting = function(ctx, meetingId, callback) {
    var allVisibilities = _.values(AuthzConstants.visibility);
    ...
    _getMeeting(meetingId, function(err, meeting) {
        ...
        AuthzPermissions.canManage(ctx, meeting, function(err) {
            ...
            MeetingsAPI.emit(MeetingsConstants.events.END_MEETING, ctx, meeting, function(errs) {
                if (errs) {
                    return callback(_.first(errs));
                }
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Meeting}    callback.recording      The updated recording object
 */
var updateRecording = module.exports.updateRecording = function(ctx, meetingId, recordingId, recording, callback) {
    var allVisibilities = _.values(AuthzConstants.visibility);

    var validator = new Validator();
    validator.check(meetingId, {'code': 400, 'msg': 'A meeting id must be provided'}).isResourceId();
    validator.check(null, {'code': 401, 'msg': 'You must be authenticated to update a recording'}).isLoggedInUser(ctx);
    validator.check(null, {'code': 400, 'msg': 'A recording id must be provided'}).isString(recordingId);
    ...
    _getMeeting(meetingId, function(err, meeting) {
        ...
        // authorize against the meeting
        AuthzPermissions.canManage(ctx, meeting, function(err) {
            if (err) {
                return callback(err);
            }

            MeetingsAPI.Bbb.updateRecordingsURL(ctx, recordingId, recording, function(err, info) {
                if (err) {
                    return callback(err);
                }
