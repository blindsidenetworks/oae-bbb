...
OAE.tenantRouter.on('post', '/api/meeting/:meetingId/open', function(req, res) {
    MeetingsAPI.Meetings.openMeeting(req.ctx, req.params.meetingId, function(err, meeting) {
        if (err) {
            return res.send(err.code, err.msg);
        }
