...
OAE.tenantRouter.on('get', '/api/meetup/:groupId/join', function(req, res) {
    PrincipalsAPI.getFullGroupProfile(req.ctx, req.params.groupId, function(err, groupProfile) {
        ...
        MeetingsAPI.Bbb.getDefaultConfigXML(req.ctx, function(err, result) {
            if(err || result.returncode != 'success') {
                ...
                MeetingsAPI.Bbb.joinURL(req.ctx, profile, xml, function(err, joinInfo) {
                    if(err) {
                        res.send(503, 'Fatal error');
                    }
...
OAE.tenantRouter.on('post', '/api/meetup/:groupId/recording', function(req, res) {
    ...
    console.info(req.body.signed_parameters);

    var secret = Config.getValue(req.ctx.tenant().alias, 'bbb', 'secret');
    console.info(secret);

    jwt.verify(req.body.signed_parameters, secret, {algorithms: ["HS256"]},function(err, decoded) {
        if (err) {
        ...
        PrincipalsAPI.getFullGroupProfile(req.ctx, req.params.groupId, function(err, groupProfile) {
            ...
            MeetingsAPI.Bbb.getRecordingsURL(req.ctx, groupProfile, function(err, getRecordings) {
              console.info('getRecordings');
              console.info(getRecordings);
...
OAE.tenantRouter.on('post', '/api/meetup/:groupId/end', function(req, res) {
    PrincipalsAPI.getFullGroupProfile(req.ctx, req.params.groupId, function(err, groupProfile) {
        ...
        MeetingsAPI.Bbb.getEndURL(req.ctx, groupProfile, function(err, end) {
            if (err) {
