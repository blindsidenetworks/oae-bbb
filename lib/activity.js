...
var log = require('oae-logger').logger('oae-meetings');
 ...
 */
MeetingsAPI.on(MeetingsConstants.events.UPDATED_MEETING, function(ctx, oldMeeting, newMeeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
 ...
 */
MeetingsAPI.on(MeetingsConstants.events.UPDATED_MEETING_MEMBERS, function(ctx, meeting, roleChanges, newPrincipalIds, updatedPrincipalIds) {
