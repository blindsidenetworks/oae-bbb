...
/*!
 * Post either a meeting-update or meeting-update-visibility activity when a user updates a meeting's metadata.
 */
MeetingsAPI.on(MeetingsConstants.events.UPDATED_MEETING, function(ctx, newMeeting, oldMeeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('meeting', newMeeting.id, {'meeting': newMeeting});
...
ActivityAPI.registerActivityType(MeetingsConstants.activity.ACTIVITY_MEETING_START, {
    'streams': {
        'activity': {
            'router': {
                'actor': ['self'],
...
/*!
 * Post a meeting-start activity when a user starts a meeting.
 */
MeetingsAPI.on(MeetingsConstants.events.START_MEETING, function(ctx, meeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('meeting', meeting.id, {'meeting': meeting});
    var activityType = MeetingsConstants.activity.ACTIVITY_MEETING_START;
 ...
 */
MeetingsAPI.on(MeetupsConstants.events.JOIN_MEETUP, function(ctx, group) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var targetResource = new ActivityModel.ActivitySeedResource('group', group.id, {'group': group});
    var meetup = group;
    meetup.profilePath += '/meetup';
    meetup.resourceType = 'meetup'
    var objectResource = new ActivityModel.ActivitySeedResource('group', meetup.id, {'group': meetup});
    var activityType = MeetupsConstants.activity.ACTIVITY_MEETUP_JOIN;

    var activitySeed = new ActivityModel.ActivitySeed(activityType, millis, ActivityConstants.verbs.POST, actorResource, objectResource, targetResource);
    ActivityAPI.postActivity(ctx, activitySeed);
});

ActivityAPI.registerActivityType(MeetupsConstants.activity.ACTIVITY_MEETUP_CLOSE, {
    'groupBy': [{'object': true}],
    'streams': {
        'activity': {
            'router': {
 ...
 * Post a meetup-join activity when a user joins a meetup.
 */
MeetingsAPI.on(MeetupsConstants.events.CLOSE_MEETUP, function(ctx, group) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('group', group.id, {'group': group});
    var activityType = MeetupsConstants.activity.ACTIVITY_MEETUP_CLOSE;
...
ActivityAPI.registerActivityType(MeetupsConstants.activity.ACTIVITY_MEETUP_END, {
    'groupBy': [{'object': true}],
    'streams': {
        'activity': {
            'router': {
 ...
 * Post a meetup-join activity when a user joins a meetup.
 */
MeetingsAPI.on(MeetupsConstants.events.END_MEETUP, function(ctx, group) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('group', group.id, {'group': group});
    var activityType = MeetupsConstants.activity.ACTIVITY_MEETUP_END;

    var activitySeed = new ActivityModel.ActivitySeed(activityType, millis, ActivityConstants.verbs.UPDATE, actorResource, objectResource);
    ActivityAPI.postActivity(ctx, activitySeed);
});

ActivityAPI.registerActivityType(MeetingsConstants.activity.ACTIVITY_MEETING_END, {
    'streams': {
        'activity': {
            'router': {
                'actor': ['self'],
...
/*!
 * Post a meeting-end activity when a user ends a meeting.
 */
MeetingsAPI.on(MeetingsConstants.events.END_MEETING, function(ctx, meeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('meeting', meeting.id, {'meeting': meeting});
    var activityType = MeetingsConstants.activity.ACTIVITY_MEETING_END;
...
/*!
 * Post a meeting-message activity when a user comments on a meeting
 */
MeetingsAPI.on(MeetingsConstants.events.CREATED_MEETING_MESSAGE, function(ctx, message, meeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('meeting-message', message.id, {'meetingId': meeting.id, 'message': message});
    var targetResource = new ActivityModel.ActivitySeedResource('meeting', meeting.id, {'meeting': meeting});
...
ActivityAPI.registerActivityType(MeetingsConstants.activity.ACTIVITY_MEETING_SHARE, {
    'groupBy': [
        // "Branden Visser shared a meeting with 5 users and groups"
        {'actor': true, 'object': true},

        // "Branden Visser shared 8 meetings with OAE Team"
...
/*!
 * Post a meeting-share or meeting-add-to-library activity based on meeting sharing
 */
MeetingsAPI.on(MeetingsConstants.events.UPDATED_MEETING_MEMBERS, function(ctx, meeting, memberChangeInfo, opts) {
    ...
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var meetingResource = new ActivityModel.ActivitySeedResource('meeting', meeting.id, {'meeting': meeting});
...
////////////////////////////////
// MEETING RECORDINGS UPDATE //
///////////////////////////////

ActivityAPI.registerActivityType(MeetingsConstants.activity.ACTIVITY_RECORDING_UPDATE, {
    'streams': {
        'activity': {
            'router': {
                'actor': ['self'],
...
/*!
 * Post a recording update activity when a manager updates a recording.
 */
MeetingsAPI.on(MeetingsConstants.events.UPDATE_RECORDING, function(ctx, meeting) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('meeting', meeting.id, {'meeting': meeting});
    var activityType = MeetingsConstants.activity.ACTIVITY_RECORDING_UPDATE;
...
var _meetingProducer = function(resource, callback) {
    var meeting = (resource.resourceData && resource.resourceData.meeting) ? resource.resourceData.meeting : null;
...
var _createPersistentMeetingActivityEntity = function(meeting) {
    return new ActivityModel.ActivityEntity('meeting', meeting.id, meeting.visibility, {'meeting': meeting});
};
...
var _meetingMessageProducer = function(resource, callback) {
    ...
    MeetingsDAO.getMeeting(meetingId, function(err, meeting) {
        ...
        MessageBoxUtil.createPersistentMessageActivityEntity(message, function(err, entity) {
            ...
            entity.objectType = 'meeting-message';
...
var _transformPersistentMeetingActivityEntity = function(ctx, entity) {
    ...
    var globalId = baseUrl + '/api/meeting/' + meeting.id;
    ...
    return new ActivityModel.ActivityEntity('meeting', globalId, meeting.visibility, opts);
};
...
var _meetingMessageTransformer = function(ctx, activityEntities, callback) {
    ...
    _.keys(activityEntities).forEach(function(activityId) {
        ...
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
            ...
            var profilePath = '/meeting/' + resource.tenantAlias + '/' + resource.resourceId;
            var urlFormat = '/api/meeting/' + meetingId + '/messages/%s';
...
ActivityAPI.registerActivityEntityType('meeting-message', {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('meeting', 'managers', function(associationsCtx, entity, callback) {
    associationsCtx.get('members-by-role', function(err, membersByRole) {
        if (err) {
            return callback(err);
        }
 ...
 */
ActivityAPI.registerActivityEntityAssociation('meeting', 'message-contributors', function(associationsCtx, entity, callback) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('meeting-message', 'self', function(associationsCtx, entity, callback) {
