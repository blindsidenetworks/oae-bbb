...
var getJoinMeetingURL = module.exports.getJoinMeetingURL = function(req, meetingProfile, callback) {
    ...
    BBBProxy.executeBBBCall(meetingInfoURL, function(err, meetingInfo) {
        ...
        if( meetingInfo.returncode == 'FAILED' && meetingInfo.messageKey == 'notFound' ) {
            //Create the meeting
            var params = {'meetingID': meetingID, 'name':meetingName, 'logoutURL': req.protocol+'://'+req.host+meetingProfile.profilePath+'/close', 'record': 'false'};
            ...
            BBBProxy.executeBBBCall(createMeetingURL, function(err, meetingInfo) {
                ...
                var password;
                if( meetingProfile.isModerator || meetingProfile.allModerators == 'true' ) {
...
var deleteRecordingsURL = module.exports.deleteRecordingsURL = function(req, recordingID, callback) {
  ...
  var recordingID = sha1(recordingID + config.secret);

  // Construct and sign the URL
  var params = {'recordingID': recordingID};
...
// var updateRecordingsURL = module.exports.updateRecordingsURL = function(req, recordingID, callback) {
//
//   var config = _getConfig(req.ctx.tenant().alias);
//
//   var recordingID = sha1(recordingID + config.secret);
...
//   // Construct and sign the URL
//   var params = {'recordingID': recordingID};
//   var recordingsURL = _getBBBActionURL(config.endpoint, 'publishRecordings', config.secret, _getQueryStringParams(params));
//   return callback(null, {'returncode':'success','url': recordingsURL});
// };
